...
The dataset is a sample from iPinYou dataset which is described in `r citet(bib[["zhang2014real"]])`.
...
The following scripts use an implementation of the FTRL-Proximal for Logistic Regresion, which is published in `r citet(bib[["DBLP:conf/kdd/McMahanHSYEGNPDGCLWHBK13"]])`, to predict the probability (1-step prediction) and update the model simultaneously.
...
If we use the same algorithm to predict the click through rate of the 3rd season of iPinYou, the AUC will be 0.77 which is comparable to the AUC reported in `r citet(bib[["zhang2014real"]])`.
