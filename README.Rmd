...
For instance, the bag-of-word representation in document classification problem requires scanning entire dataset to know how many words there are, i.e. the dimension of the feature matrix.

Hashing trick have other purposes like cross-product features where two or more features are combined to create a new one. Another well known use is dimension reduction when you setup the hash function such that the output range is significantly smaller than the number of unique features in your original dataset.
...
If we use the same algorithm to predict the click through rate of the 3rd season of iPinYou, the overall AUC will be 0.77 which is comparable to the overall AUC of 3rd season 0.76 reported in `r citep(bib[["zhang2014real"]])`.
