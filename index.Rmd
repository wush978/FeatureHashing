...
```{r gdbt.echo, eval = FALSE, echo = TRUE}
suppressPackageStartupMessages({
  library(xgboost)
  })
...
```{r gdbt, eval = TRUE, cache=TRUE, results='markup'}
suppressPackageStartupMessages({
  library(xgboost)
  })
log <- capture.output(cv.g.gdbt <- xgboost(m.train, imp.train$is_click, max.depth = 7, eta = 0.1, 
                     nround = 100, objective = "binary:logistic")
               )
cat(head(log), sep="\n")
cat(tail(log), sep="\n")
