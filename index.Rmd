...
<center><small><http://www.meetup.com/Taiwan-R></small></center>
...
```{r tm, dependson = "imdb.split", echo = TRUE}
...
retval.time <- numeric(length(hash_size.set))
for(i in seq_along(hash_size.set)) {
  retval.time[i] <- system.time({
...
rm(criteo.train, criteo.test, y.train, y.test, x)
```

```{r cons1.plot1, fig.width = 10, fig.height = 7, dependson = "cons1"}
x <- 10:22
plot(x, retval, type = "l", xaxt = "n", xlab = "hash size", ylab = "log likelihood")
...
```{r cons1.plot2, fig.width = 10, fig.height = 7, dependson = "cons1"}
x <- 10:22
plot(x, retval.time, type = "l", xaxt = "n", xlab = "hash size", ylab = "computation time")
axis(1, at = x, labels = parse(text = sprintf("2^%d", x)))
```
